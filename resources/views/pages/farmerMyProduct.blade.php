	     ...
	     @foreach ($pendingRequestData as $data)
	        ...
	        @if ($data->product_catagory == $distinctCatagory[$i]["product_catagory"])
	          ...
	          <div class="col-sm-4 thumbnail" style="height: 690px;">
	            ...
	            <h4 style="display: inline-block;padding-left: 60px;">To : </h4> <strong>{{ $data->product_available_to }}</strong> <br><br>

	            <a href="{{ route('farmer.update_product', ['id' => $data->id]) }}"><button type="submit" class="btn btn-info pull-right">Edit Product Info</button></a>

	            <a href="{{ route('farmer.delete_product', ['id' =>$data->id]) }}">
